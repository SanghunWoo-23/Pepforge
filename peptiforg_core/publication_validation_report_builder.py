...
from peptiforg_core.component_versions import PUBLICATION_REPORT_BUILDER_VERSION
...
def build_claim_guard_table() -> list[dict[str,str]]:
    return [
        ...
        {"claim_type":"full_MD","unsafe_claim":"Pepforge performed full publication-grade MD.","status":"blocked","safe_expression":"Pepforge V4 generates preparation/report packages and can import external MD summary metadata; production MD and trajectory analysis are V5 or external-tool scope.","required_evidence":"OpenMM/GROMACS/AMBER/NAMD protocol, force field, runtime, convergence, and independent trajectory analysis"},
        {"claim_type":"external_engine_replacement","unsafe_claim":"Pepforge replaces AutoDock Vina, GROMACS, AMBER, or OpenMM.","status":"blocked","safe_expression":"Pepforge V4 is a preparation, screening, static-structure review, validation-bridge, and reporting workbench; it does not silently substitute for external simulation engines.","required_evidence":"not applicable; do not claim replacement"},
...
def compute_publication_readiness(evidence_report, md_summary, docking_rows):
    ...
    if md_summary:
        score += 1
        warnings.append(
            "External MD metadata are present, but nominal duration and summary RMSD/contact values are not treated as a convergence grade. "
            "Use the V5 simulation workflow or external trajectory analysis with independent replicate/convergence diagnostics for stronger MD support."
        )
    else:
        warnings.append("External MD evidence is absent or not imported.")
