...
from spps_v4_gui.recommendation import loading as loading_recommendation
from spps_v4_gui.recommendation import cleavage as cleavage_recommendation
from spps_v4_gui.recommendation import coupling as coupling_recommendation
from spps_v4_gui.recommendation import models as recommendation_models
from spps_v4_gui.recommendation.provenance import attach_evidence_trace
...
def compare_conditions(gui: Any, scenario_a: dict[str, Any], scenario_b: dict[str, Any]) -> dict[str, Any]:
    from spps_v4_gui.decision_support import compare_scenarios
...
def _active_item_and_run(gui: Any, *, save_editor: bool = True):
    ...
    """
    from spps_v4_gui import data_system
    from spps_v4_gui.modules import gui_common
    if save_editor:
        gui_common.save_active(gui)
    items = list(getattr(gui, "pm_items", []) or [])
...
def active_run_context(gui: Any) -> dict[str, str]:
    ...
    try:
        ...
        # Preserve the Pepforge R9 compact contract for legacy runs while exposing
        # V6 lifecycle/provenance fields whenever they are actually populated.
        for key in ("repeat_of_run_id", "status", "started_at", "completed_at"):
...
def preflight_check(gui: Any) -> dict[str, Any]:
    ...
    from spps_v4_gui.preflight import check_snapshot
    item, run = _active_item_and_run(gui)
    if item is None or run is None:
...
def repeat_experiment(gui: Any, *, scale_mmol: Any = None, name: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            from spps_v4_gui.error_policy import log_nonfatal
...
def export_run_package(gui: Any, output_zip: str | Path) -> Path:
    ...
    from spps_v4_gui import run_package
    item, run = _active_item_and_run(gui)
    if item is None or run is None:
...
def start_experiment(gui: Any) -> dict[str, Any]:
    ...
    if not was_started:
        from spps_v4_gui.preflight import check_snapshot, format_report
...
def execution_summary(gui: Any) -> dict[str, Any]:
    from spps_v4_gui.modules import gui_common
    from spps_v4_gui.decision_support import execution_summary as summarize
    gui_common.save_active(gui)
...
def add_loading_record(gui: Any, values: dict[str, Any], *, status: str = "verified"):
    saved = experimental_data.add_record("loading", _attach_run_context(gui, values), _ready_db(gui), status=status)
    return _require_saved_record("loading", saved, _ready_db(gui))


def add_cleavage_record(gui: Any, values: dict[str, Any], *, status: str = "verified"):
    saved = experimental_data.add_record("cleavage", _attach_run_context(gui, values), _ready_db(gui), status=status)
    return _require_saved_record("cleavage", saved, _ready_db(gui))
