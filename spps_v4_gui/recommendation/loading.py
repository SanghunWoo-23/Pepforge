...
def loading_recommendation(*args, **kwargs):
    # Pepforge shares advisor-form extras (aa/base/time/apply flags) that the
    # bounded history selector intentionally does not accept. Keep those extras
    # for the V6 target/inverse layer, but pass only its narrow API downstream.
    history_kwargs = {k: kwargs[k] for k in ("target_loading_mmol_g", "db_path", "include_parsed") if k in kwargs}
    if args:
        result=dict(history_engine.loading_recommendation(*args, **history_kwargs))
    else:
        result=dict(history_engine.loading_recommendation(
            kwargs.get("resin", ""), kwargs.get("amino_acid", ""), **history_kwargs
        ))
    resin=kwargs.get('resin', args[0] if len(args)>0 else '')
    amino=kwargs.get('amino_acid', args[1] if len(args)>1 else '')
