...
def sequence_difficulty_map(sequence: str) -> dict[str,Any]:
    ...
    try:
        ...
        aggregation_evidence = literature_aggregation_evidence(sequence)
    except Exception as exc:
        aggregation_evidence = {'status':'unavailable','reason':str(exc),'automatic_plan_change_allowed':False}
    return {
        'positions':positions,
        'max_score':max([p['score'] for p in positions],default=0),
        'score_kind':'deterministic review score',
        'warnings':list(getattr(parsed,'warnings',[]) or []),
        'apply_allowed':False,
        'literature_aggregation_evidence':aggregation_evidence,
    }
...
def stage_risk_advisor(*, sequence:str='', product:str='', resin:str='', scale_mmol:Any=None,
    ...
    try:
        from spps_v4_gui.aggregation_evidence_v5 import literature_aggregation_evidence
        aggregation_evidence = literature_aggregation_evidence(seq)
    except Exception as exc:
        aggregation_evidence = {'status':'unavailable','reason':str(exc),'automatic_plan_change_allowed':False}
    aggregation_status = aggregation_evidence.get('status')
    if aggregation_status in {'available', 'partial_out_of_domain'}:
        frac = aggregation_evidence.get('composition_fraction_S_V_I_T')
        npos = len(aggregation_evidence.get('resin_side_2_to_12_pseudoproline_candidate_positions') or [])
        coupling_reasons.append(
            f"Literature-only AFPS aggregation review ({aggregation_status}): canonical-L subset "
            f"{aggregation_evidence.get('canonical_residue_count', 0)}/{aggregation_evidence.get('core_residue_count', 0)}; "
            f"S/V/I/T fraction within that subset={frac}; resin-side S/T mitigation-review positions={npos}. "
            "Out-of-domain positions are excluded and this does not change the synthesis plan automatically."
        )
    elif aggregation_status == 'out_of_domain':
        coupling_reasons.append(
            "Literature-only AFPS aggregation review: current core is outside the canonical-L evidence domain; "
            "no composition fraction or automatic synthesis-plan change is inferred."
        )
    ...
    return {
        ...
        'literature_aggregation_evidence':aggregation_evidence,
        'risk_kind':'deterministic/evidence review; not failure probability',
        'apply_allowed':False,
    }
